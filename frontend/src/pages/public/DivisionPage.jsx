...
const DivisionPage = () => {
  ...
  const fetchData = async () => {
    try {
      const [srRes, leadersRes, articlesRes] = await Promise.all([
        api.get(`/sub-regions/${id}`),
        api.get(`/leaders?sub_region_id=${id}`),
        api.get(`/articles?sub_region_id=${id}&status=published`),
      ]);
      setSubRegion(srRes.data);
    ...
    return (
      <div className="min-h-screen bg-stone-50">
        ...
        <div className="pt-24 text-center py-16">
          <p className="text-stone-500">Division not found</p>
  ...
  return (
    <div className="min-h-screen bg-stone-50">
      ...
      {/* Hero */}
      <section className="pt-24 pb-12 bg-gradient-to-br from-green-700 to-green-800" data-testid="division-hero">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {/* Breadcrumb */}
          <nav className="flex items-center gap-2 text-green-200 text-sm mb-6 flex-wrap" data-testid="breadcrumb">
            <Link to="/" className="hover:text-white flex items-center gap-1">
              <Home className="w-4 h-4" />
              Home
            </Link>
            <ChevronRight className="w-4 h-4" />
            <Link to={`/constituency/${subRegion.parent_constituency_id}`} className="hover:text-white">
              {subRegion.parent_constituency_name}
            </Link>
            <ChevronRight className="w-4 h-4" />
            <span className="text-white">{subRegion.name}</span>
          </nav>

          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-green-200 mb-2">{subRegion.type}</p>
              <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-white">{subRegion.name}</h1>
              {subRegion.description && (
                <p className="text-green-100 mt-4 max-w-2xl">{subRegion.description}</p>
              )}
            </div>
      ...
      <FeaturedVideosSection 
        videos={subRegion.video_links || []}
        title="Featured Videos"
        subtitle={`Explore ${subRegion.name} digitally`}
        areaName={subRegion.name}
        themeColor="green"
      />

      {/* Local Leaders */}
      {leaders.length > 0 && (
        <section className="py-12 bg-white border-b border-stone-200" data-testid="local-leaders">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-xl font-bold text-stone-900 mb-6">Local Leaders</h2>
      ...
      {/* Activity Feed */}
      <section className="py-12" data-testid="activity-feed">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {articles.length > 0 ? (
            ...
            <div className="text-center py-12 bg-white rounded-xl border border-stone-200">
              ...
              <p className="text-stone-500">No development works posted for this area yet.</p>
            </div>
          )}
        </div>
      </section>
